...
resource "alicloud_ram_role" "this" {
  ...
  document    = "${file(join("", list(var.ram_role_default_name, ".json")))}"
...
resource "alicloud_ram_policy" "this" {
  ...
  document    = "${file(join("", list(var.ram_policy_default_name, ".json")))}"
...
resource "alicloud_actiontrail" "this" {
  ...
  sls_project_arn    = "${var.log_project_arn != "" ? var.log_project_arn : var.log_project_name == "" ? "" : join("/", list(join(":", list("acs:log", data.alicloud_regions.default.regions.0.id, data.alicloud_account.default.id, "project")), join("", alicloud_log_project.this.*.name)))}"
